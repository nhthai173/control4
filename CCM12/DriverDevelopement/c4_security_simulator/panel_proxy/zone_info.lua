...
function ZoneInformation:construct(ZoneID)
	...
	self._ZoneName = "Zone " .. tostring(ZoneID)
...
function ZoneInformation:ZoneInfoChanged()
	LogTrace("ZoneInfoChanged => Zone %d: Name = %s, Type = %d, Type_Alt = %d", tonumber(self._ZoneID), tostring(self._ZoneName), tonumber(self._ZoneTypeID_C4), tonumber(self._ZoneTypeID_3P))
...
function ZoneInformation:SetZoneState(IsOpen, Initializing)
	...
	if (self._IsOpen ~= IsOpen) then
		...
		if(not JustInitializing) then
			...
			LogTrace("!!!!!!   Zone %d %s  !!!!!!", tonumber(self._ZoneID), tostring(self:GetZoneState()))
			...
			NOTIFY.PANEL_ZONE_STATE(self._ZoneID, self._IsOpen, TheSecurityPanel._BindingID)
		end
	end
end
