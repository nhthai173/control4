...
function DeviceConnectionBase:StartCommandTimer(...)
	...
	if (self._ExpectAck) then
		...
		LogTrace("Starting wait Timer: %d for %s", self._WaitResponseTimer._timerID, command_name)
	else
		...
		LogTrace("Starting Send Timer:  %d for %s", self._SendTimer._timerID, command_name)
	end
end
...
function DeviceConnectionBase.OnSendTimeExpired(Instance)
	...
	local tCommand = Instance._CommandQueue:pop()
	LogTrace("Send Timer expired - Pop Command:" .. tCommand.command_name .. ", Send Next Command")
...
function DeviceConnectionBase.OnWaitTimeExpired(Instance)
	...
	if (Instance._CommandRetryCount >= Instance._RetryCountMax) then
		...
		local cmd = Instance._CommandQueue:pop()
...
function DeviceConnectionBase:HandleACK()
	local cmd = self._CommandQueue:pop()
...
function DeviceConnectionBase:QueueCommand(sCommand, ...)
	...
	if (self._CommandQueue:empty()) then
...
function DeviceConnectionBase:SendNextCommand()
	...
	if (not self._CommandQueue:empty()) then
		local tCommand = self._CommandQueue:value()
...
function ReceivedFromSerial(idBinding, sData)
	CCM.ReceivedFromSerial(idBinding, sData)
